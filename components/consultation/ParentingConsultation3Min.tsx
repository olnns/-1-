...
/** 육아용품 등에서 플로팅 상담을 열 때 */
export const CONSULTATION_DOCK_OPEN_EVENT = "momoA-consultation-dock-open";

export type ParentingConsultation3MinProps = {
  /** dock: 우측 하단 날개(마이페이지 제외 탭에서만 마운트) */
  layout?: "inline" | "dock";
};
...
const pillChoiceSelected =
  "border-[#FF853E] bg-[#FFF8F4] shadow-md ring-2 ring-[#FF853E]/30 hover:border-[#FF853E] hover:bg-[#FFF3E8]";
...
export default function ParentingConsultation3Min({ layout = "inline" }: ParentingConsultation3MinProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(CONSULTATION_DOCK_OPEN_EVENT, open);
    return () => window.removeEventListener(CONSULTATION_DOCK_OPEN_EVENT, open);
  }, [isDock]);

  useEffect(() => {
    if (!isConsultationActive || isDock) return;
    const prev = document.body.style.overflow;
    document.body.style.overflow = "hidden";
    return () => {
      document.body.style.overflow = prev;
    };
  }, [isConsultationActive, isDock]);
  const [q1, setQ1] = useState<PainMomentId | null>(null);
  const [q2, setQ2] = useState<string | null>(null);
  const [q3, setQ3] = useState<string | null>(null);
  /** STEP 3: 선택 후 결과로 확정하기 전까지 보관 (같은 화면에서 답 변경 가능) */
  const [q4Draft, setQ4Draft] = useState<string | null>(null);
  ...
  /** 설문 단계별 뒤로가기 (선택값은 유지해 같은 단계에서 바꿀 수 있음) */
  const goBackSurveyStep = useCallback(() => {
    if (step === 4) {
      setQ4Draft(null);
      setStep("scene");
      return;
  ...
  }, [step, goBackToChooseMode]);

  /** 헤더 뒤로가기: 챗 ↔ 방식 선택, 설문은 단계별 */
  const goBackInConsultationHeader = useCallback(() => {
    if (step === "chat") goBackToChooseMode();
    else goBackSurveyStep();
  }, [step, goBackToChooseMode, goBackSurveyStep]);
    ...
    () => (
      <div className="relative overflow-hidden rounded-[1.75rem] bg-[#FFF7F0]">
        ...
        <div className="relative px-5 pb-8 pt-7 sm:px-8 sm:pb-10 sm:pt-9">
          ...
          >
            3분
            <span className="font-black tracking-tight text-sky-600">AI</span>
            상담
          </span>
            ...
            <div className="min-w-0 flex-1">
              ...
              <h2 className="mt-1 text-[1.05rem] font-bold leading-[1.38] tracking-tight text-slate-900 sm:text-[1.2rem] sm:leading-[1.34]">
                ...
                <span className="inline-block text-[1.1rem] leading-[1.36] sm:text-[1.28rem] sm:leading-[1.32]">
                  <span className="text-[#FF853E]">지금 필요한 정보</span>
                  만 골라볼까요?
                </span>
              </h2>
            </div>
  ...
  const chooseOuter =
    isDock && dockOpen
      ? "absolute inset-0 z-[12] flex min-h-0 w-full flex-col overflow-hidden rounded-t-xl bg-[#FF853E] px-3 pb-0 pt-3"
      : "fixed left-1/2 z-[35] flex w-full max-w-[430px] -translate-x-1/2 flex-col rounded-t-none bg-[#FF853E] px-3 pb-0 pt-3 top-[calc(env(safe-area-inset-top,0px)+5rem)] bottom-[calc(5.5rem+env(safe-area-inset-bottom,0px))]";

  const mainOuter =
    isDock && dockOpen
      ? "absolute inset-0 z-[12] flex min-h-0 w-full flex-col overflow-hidden rounded-t-2xl bg-[#FF853E] px-3 pb-0 pt-3"
      : "fixed left-1/2 z-[35] flex w-full max-w-[430px] -translate-x-1/2 flex-col rounded-t-2xl bg-[#FF853E] px-3 pb-0 pt-3 top-[calc(env(safe-area-inset-top,0px)+5rem)] bottom-[calc(5.5rem+env(safe-area-inset-bottom,0px))]";

  function chooseModeDialog() {
    return (
      <div className={chooseOuter} role="dialog" aria-modal="true" aria-label="상담 방식 선택">
  ...
  function mainConsultationDialog() {
    return (
      <div className={mainOuter} role="dialog" aria-modal="true" aria-label="3분 AI 상담">
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain rounded-t-xl bg-[#FFF8F5] pb-[max(0.75rem,env(safe-area-inset-bottom,0px))]">
          <div
            className={`relative overflow-hidden rounded-t-xl ${
              step === "result" ? "bg-[#FFF5ED]" : "bg-[#FFF8F5]"
            }`}
          >
              ...
              {step !== "result" && (
                <>
                  <div className="flex flex-wrap items-center gap-2">
                    {(step === 1 || step === "scene" || step === 4 || step === "chat") && (
                      <button
                        type="button"
                        onClick={goBackInConsultationHeader}
                        className="inline-flex h-10 shrink-0 items-center gap-0.5 rounded-xl border border-[#FFD2BF]/80 bg-white/95 px-2 text-[#C2410C] shadow-sm transition hover:bg-white active:scale-[0.98]"
                        aria-label={
                          step === "chat"
                            ? "상담 방식 선택 화면으로 돌아가기"
                            : step === 1
                      ...
                      >
                        ...
                        <span className="hidden pr-0.5 text-[13px] font-bold sm:inline">이전</span>
                      </button>
                    )}
                    <div className="flex min-w-0 flex-1 flex-wrap items-center justify-between gap-2">
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-[#FFF1EA] px-4 py-1.5 text-[13px] font-bold text-[#C2410C] ring-2 ring-[#FF853E]/35 shadow-sm sm:px-4 sm:py-2 sm:text-[14px]">
                        3분
                        <span className="font-bold text-sky-600">AI</span>
                        상담
                      </span>
                      {showProgress && (
                  ...
                  <h2 className="mt-4 text-[1.15rem] font-bold leading-snug tracking-tight text-slate-900 sm:text-[1.25rem]">
                    {(step === 1 || (step === "chat" && !chatSlots.q1)) && (
                      <>
                        STEP 1 · <span className="text-[#FF853E]">지금</span> 에너지가 가장 많이 쓰이는 순간은?
                      </>
                    )}
                    {(step === "scene" || (step === "chat" && chatSlots.q1 && (!chatSlots.q2 || !chatSlots.q3))) &&
                      (step === "scene" ? q1 : chatSlots.q1) && (
                        <>
                          STEP 2 · 그때 가장 가까운 장면을 <span className="text-[#FF853E]">하나만</span> 골라주세요
                        </>
                      )}
            ...
            <div className="relative px-5 pb-7 pt-2 sm:px-7 sm:pb-8">
              ...
              {step === 1 && (
                <div className="mx-auto max-w-lg space-y-3">
                  <p className="text-[11px] font-medium text-slate-500">
                    선택한 항목은 주황색으로 표시돼요. 다른 항목을 누르면 바로 바뀌어요.
                  </p>
                  <div className="flex flex-col gap-2.5">
                    {Q1_OPTIONS.map((o) => (
                      <button
                        ...
                        className={`${pillChoiceLight} ${q1 === o.id ? pillChoiceSelected : ""}`}
                      >
                        {o.text}
                      </button>
                    ))}
              ...
              {step === "scene" && q1 && (
                <div className="mx-auto max-w-lg space-y-3">
                  <p className="text-[11px] font-medium text-slate-500">
                    장면을 고른 뒤에도 상단「이전」으로 돌아와 다른 장면을 고를 수 있어요.
                  </p>
                  <div className="flex flex-col gap-2.5">
                    {sceneList.map((o) => {
                      const chosen = q2 === o.q2 && q3 === o.q3;
                      return (
                        <button
                          key={o.q2 + o.q3}
                          type="button"
                          ...
                          className={`${pillChoiceLight} ${chosen ? pillChoiceSelected : ""}`}
                        >
                          {o.text}
                        </button>
                      );
              ...
              {step === 4 && q1 && q2 && q3 && (
                <div className="mx-auto max-w-lg space-y-3">
                  <p className="text-[11px] font-medium text-slate-500">
                    항목을 탭해 선택한 뒤「이 선택으로 결과 보기」를 눌러 주세요. 바꾸려면 다른 항목을 누르면 돼요.
                  </p>
                  <div className="flex flex-col gap-2.5">
                    {Q4_OPTIONS.map((o) => (
                      <button
                        key={o.id}
                        type="button"
                        aria-pressed={q4Draft === o.id}
                        onClick={() => setQ4Draft(o.id)}
                        className={`${pillChoiceLight} ${q4Draft === o.id ? pillChoiceSelected : ""}`}
                      >
                        {o.text}
                      </button>
                    ))}
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (!q4Draft) return;
                      applyResult({ q1, q2, q3, q4: q4Draft });
                    }}
                    className="w-full rounded-2xl bg-[#FF853E] py-3.5 text-sm font-bold text-white shadow-md transition hover:brightness-[1.03] disabled:cursor-not-allowed disabled:opacity-40"
                  >
                    이 선택으로 결과 보기
                  </button>
                </div>
  ...
  return (
      ...
      {isDock && (
        <>
          {dockOpen && (
            <button
              type="button"
              className="fixed inset-0 z-[44] bg-slate-900/25 backdrop-blur-[0.5px]"
              aria-label="상담 창 닫기"
              onClick={() => setDockOpen(false)}
            />
          )}
          <div className="pointer-events-none fixed bottom-[calc(6.6rem+env(safe-area-inset-bottom,0px))] left-1/2 z-[45] flex w-full max-w-app -translate-x-1/2 flex-col items-end gap-2 pr-0 sm:pr-0">
            {dockOpen && (
              <div className="pointer-events-auto mb-1 flex max-h-[min(78dvh,540px)] min-h-[14rem] w-[min(23rem,calc(100%-0.5rem))] flex-col overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-[0_14px_44px_-10px_rgba(15,23,42,0.28)] ring-1 ring-black/[0.04]">
                <div className="flex shrink-0 items-center justify-between gap-2 border-b border-slate-100 bg-white px-3 py-2.5">
                  <p className="text-[15px] font-bold text-slate-900">모모아 AI 상담</p>
                  <button
                    type="button"
                    onClick={() => setDockOpen(false)}
                ...
                <div className="relative min-h-0 flex-1 overflow-y-auto overscroll-contain bg-[#FFFCFA] p-3">
                  <div className="relative min-h-[min(48dvh,400px)]">
                    {step === "intro" && introBlock}
                    {isConsultationActive && step === "chooseMode" && chooseModeDialog()}
                    {isConsultationActive && step !== "chooseMode" && mainConsultationDialog()}
                  </div>
                </div>
              </div>
            )}
            <button
              ...
              className="-mr-5 pointer-events-auto flex h-[5.9rem] w-[5.9rem] flex-col items-center justify-center rounded-full border border-[#F08A45] bg-[#FF8A3D] px-2 pb-1.5 pt-1.5 shadow-[0_16px_28px_-16px_rgba(245,115,45,0.65)] ring-1 ring-[#FFD9C3]/80 transition hover:-translate-y-0.5 hover:bg-[#FF7E2A] hover:shadow-[0_20px_30px_-14px_rgba(245,115,45,0.52)] active:translate-y-0"
              aria-expanded={dockOpen}
              aria-label={dockOpen ? "AI 상담 닫기" : "AI 상담 열기"}
            >
              <div className="relative flex w-full flex-col items-center">
                {!dockOpen && saved && (
                  <span className="absolute -right-0.5 -top-1 flex h-[1.125rem] min-w-[1.125rem] items-center justify-center rounded-full bg-red-500 px-1 text-[9px] font-bold leading-none text-white ring-2 ring-white">
                    N
                  </span>
                )}
                <div className="flex h-[3rem] w-[3rem] shrink-0 items-center justify-center">
                  <svg
                    viewBox="0 0 24 24"
                    className="h-[1.75rem] w-[1.75rem] text-white"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="1.8"
                    strokeLinecap="round"
                    ...
                    aria-hidden
                  >
                    <path d="M12 3.8l1.9 4.6 4.9 1.6-4.9 1.6-1.9 4.6-1.9-4.6-4.9-1.6 4.9-1.6L12 3.8Z" />
                    <path d="M18.2 4.5v2.8M19.6 5.9h-2.8M5 16.8v2.2M6.1 17.9H3.9" opacity="0.95" />
                  </svg>
                </div>
                <span className="mt-0.5 text-center text-[12px] font-bold leading-tight tracking-[0.01em] text-white [word-break:keep-all]">
                  AI 상담
                </span>
              </div>
            </button>
    ...
    <section className={step === "intro" ? "mb-8" : ""} aria-label="3분 AI 육아용품 우선순위 상담">
      ...
      {/* 상담 방식 선택 · 진행 — 인라인(상담 탭)에서만 전체 화면 */}
      {!isDock && isConsultationActive && step === "chooseMode" && chooseModeDialog()}
      {!isDock && isConsultationActive && step !== "chooseMode" && mainConsultationDialog()}
    </section>
      )}
    </>
  );
